...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'submit_complaint') {
        ...
        $complaint_number = rand(time(), 100000000);
        ...
        if ($ComplaineeAddress && $ComplainantUID && $ComplainantAddress && $ComplaintStatus && $complaint && $description && $proof) {
            ...
            $sql = "INSERT INTO complaints (complaint_number, complainee, complaint, description, proof, filed_date, complaineeAddress, complainantUID, complainantName, complainantAddress, status) VALUES (?, ?, ?, ?, ?, NOW(), ?, ?, ?, ?, ?)";

            if ($stmt = $conn->prepare($sql)) {
                // Bind the parameters to the SQL query
                $stmt->bind_param("ssssssssss", $complaint_number, $complainee, $complaint, $description, $proof, $ComplaineeAddress, $ComplainantUID, $ComplainantName, $ComplainantAddress, $ComplaintStatus);
